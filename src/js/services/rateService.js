...
RateService.prototype.updateRates = function() {
  ...
  var rateServiceUrl = 'https://rates.litecoinz.org';

  function getLTZ(cb, tries) {
    tries = tries || 0;
    if (!self.httprequest) return;
    if (tries > 5) return cb('could not get LTZ rates');
...
RateService.prototype.toFiat = function(satoshis, code, chain) {
  ...
  return satoshis * this.SAT_TO_LTZ * this.getRate(code, chain);
};
...
RateService.prototype.fromFiat = function(amount, code, chain) {
  ...
  return amount / this.getRate(code, chain) * this.LTZ_TO_SAT;
};
