...
angular.module('copayApp.services').factory('incomingData', function($log, $state, $timeout, $ionicHistory, bitcoreLtz, $rootScope, payproService, scannerService, appConfigService, popupService, gettextCatalog) {
  ...
  root.redir = function(data) {
    ...
    function sanitizeUri(data) {
      ...
      // mobile devices, uris like litecoinz://glidera
      newUri.replace('://', ':');
    ...
    function checkPrivateKey(privateKey) {
      try {
        new bitcoreLtz.PrivateKey(privateKey, 'livenet');
      } catch (err) {
        return false;
      }
      return true;
    ...
    if ((/^litecoionz(cash)?:\?r=[\w+]/).exec(data)) {
      data = decodeURIComponent(data.replace(/litecoionz(cash)?:\?r=/, ''));
      $state.go('tabs.send', {}, {
        'reload': true,
        'notify': $state.current.name == 'tabs.send' ? false : true
      }).then(function() {
    ...
    if (bitcoreLtz.URI.isValid(data)) {
        var coin = 'ltz';
        var parsed = new bitcoreLtz.URI(data);
      ...
      // Plain Address
    } else if (bitcoreLtz.Address.isValid(data, 'livenet') || bitcoreLtz.Address.isValid(data, 'testnet')) {
