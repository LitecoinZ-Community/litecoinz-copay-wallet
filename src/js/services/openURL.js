  ...
  root.init = function() {
    ...
    if (platformInfo.isChromeApp) {
      ...
      gui.App.on('open', function(pathData) {
        if (pathData.indexOf(/^litecoinz(cash)?:/) != -1) {
          $log.debug('LitecoinZ URL found');
          handleOpenURL({
            url: pathData.substring(pathData.indexOf(/^litecoinz(cash)?:/))
          });
      ...
      if (navigator.registerProtocolHandler) {
        ...
        navigator.registerProtocolHandler('web+litecoinz', url, 'Copay LitecoinZ Handler');
