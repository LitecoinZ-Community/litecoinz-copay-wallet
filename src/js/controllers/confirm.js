...
angular.module('copayApp.controllers').controller('confirmController', function($rootScope, $scope, $interval, $filter, $timeout, $ionicScrollDelegate, gettextCatalog, walletService, platformInfo, lodash, configService, $stateParams, $window, $state, $log, profileService, bitcoreLtz, txFormatService, ongoingProcess, $ionicModal, popupService, $ionicHistory, $ionicConfig, payproService, feeService, bwcError, txConfirmNotification, externalLinkService) {
  ...
  $scope.$on("$ionicView.beforeEnter", function(event, data) {
    ...
    try {
      ...
      var message = gettextCatalog.getString('Copay only supports LitecoinZ using new version numbers addresses');
  ...
  function showSendMaxWarning(wallet, sendMaxInfo) {
    ...
    var msg = gettextCatalog.getString("{{fee}} will be deducted for litecoinz networking fees.", {
  ...
  $scope.approve = function(tx, wallet, onSendStatusChange) {
    ...
    if ($scope.paymentExpired) {
      popupService.showAlert(null, gettextCatalog.getString('This litecoinz payment request has expired.'));
      $scope.sendStatus = '';
      $timeout(function() {
        $scope.$apply();
      });
