...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    exec: {
      ...
      macos: {
        command: 'cp util/build-macos.sh webkitbuilds/. && sh webkitbuilds/build-macos.sh sign'
      },
      coveralls: {
      ...
      androidsign: {
        command: 'rm -f cordova/project/platforms/android/build/outputs/apk/android-release-signed-aligned.apk; jarsigner -verbose -sigalg SHA1withRSA -digestalg SHA1 -keystore ../litecoinz.keystore -signedjar cordova/project/platforms/android/build/outputs/apk/android-release-signed.apk  cordova/project/platforms/android/build/outputs/apk/android-release-unsigned.apk copay_play && ../android-sdk-macosx/build-tools/21.1.1/zipalign -v 4 cordova/project/platforms/android/build/outputs/apk/android-release-signed.apk cordova/project/platforms/android/build/outputs/apk/android-release-signed-aligned.apk ',
        stdin: true,
      },
      desktopsign: {
          ...
          'CFBundleURLTypes': [
            {
              ...
              'CFBundleURLSchemes': ['litecoinz', '<%= pkg.name %>']
            }
          ]
        }
      },
  ...
  grunt.registerTask('osx', ['prod', 'nwjs', 'exec:macos']);
